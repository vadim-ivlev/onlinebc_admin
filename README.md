...
Содержит файлы отвечающие за работу с базой данных и редис. Формируют запросы к базам данных и форматирование ответов в JSON.

Приложение не использует библиотек ORM. JSON форматирование в части REST API возложено на представления и функции postgres. Таким образом снижается трафик между БД и приложением, ускоряются запросы, снижается нагрузка на  хостирующий сервер, и уменьшается объем кода. 
